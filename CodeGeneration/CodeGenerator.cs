...
namespace Compiler.CodeGeneration
{
    ...
    public class CodeGenerator
    {
        ...
        private void GenerateCodeForWhileCommand(WhileCommandNode whileCommand)
        {
            ...
            Address WendJumpAddress = code.NextAddress;
            code.AddInstruction(OpCode.JUMP, Register.CB, 0, 0);
            GenerateCodeFor(whileCommand.Wend);
            code.PatchInstructionToJumpHere(WendJumpAddress);
            return;
        }

        private void GenerateCodeForDoCommand(DoCommandNode doCommand)
        {
            Debugger.Write("Generating Code for the Do Command");
            Address jumpAddress = code.NextAddress;
            code.AddInstruction(OpCode.JUMP, Register.CB, 0, 0);
            Address loopAddress = code.NextAddress;
            GenerateCodeFor(doCommand.Command);
            code.PatchInstructionToJumpHere(jumpAddress);
            Address WendWhileJumpAddress = code.NextAddress;
            code.AddInstruction(OpCode.JUMP, Register.CB, 0, 0);
            GenerateCodeFor(doCommand.WendWhile);
            code.PatchInstructionToJumpHere(WendWhileJumpAddress);
            return;
